...
try:
    _STACK_UNVAILABLE_REASON = u''
    _interface_name = u'vcan0'
    import isotp
    import can
    s = isotp.socket()
    ...
    _STACK_UNVAILABLE_REASON = unicode(e)
...
class TestIsoTPSocketConnection(UdsTest):
    ...
    def test_open(self):
        conn = IsoTPSocketConnection(interface=u'vcan0', rxid=0x001, txid=0x002, tpsock=self.tpsock1, name=u'unittest')
    ...
    def test_transmit(self):
        conn1 = IsoTPSocketConnection(interface=u'vcan0', rxid=0x100, txid=0x101, tpsock=self.tpsock1, name=u'unittest')
        conn2 = IsoTPSocketConnection(interface=u'vcan0', rxid=0x101, txid=0x100, tpsock=self.tpsock2, name=u'unittest')

        with conn1.open():
            with conn2.open():
                payload1 = "\x00\x01\x02\x03\x04"
                conn1.send(payload1)
...
class TestSocketConnection(UdsTest):
    ...
    def setUp(self):
        ...
        self.server_sock.bind((u'127.0.0.1', 0))
        ...
        while not self.thread_started:
            if (time.time() - t1) > 0.5:
                raise RuntimeError(u'Timeout while connecting sockets together.')
            time.sleep(0.01)
        time.sleep(0.01)
    ...
    def test_open(self):
        conn = SocketConnection(self.sock1, name=u'unittest')
        self.assertFalse(conn.is_open())
        conn.open()
        self.assertTrue(conn.is_open())
        conn.close()
    ...
    def test_transmit(self):
        conn1 = SocketConnection(self.sock1, name=u'unittest')
        conn2 = SocketConnection(self.sock2, name=u'unittest')
...
class TestQueueConnection(UdsTest):
    def setUp(self):
        self.conn = QueueConnection(name=u'unittest')
        self.conn.open()
...
class TestPythonIsoTpConnection(UdsTest):
    def __init__(self, *args, **kwargs):
        ...
        if not hasattr(self.__class__, u'_next_id'):
    ...
    def make_bus(self):
        return can.interface.Bus(bustype=u'socketcan', channel=u'vcan0', bitrate=500000, receive_own_messages=True)
    ...
    def setUp(self):
        ...
        self.conn = PythonIsoTpConnection(isotp.CanStack(bus=self.vcan0_bus, address=addr), name=u'unittest')
        self.conn.open()
    ...
    def test_receive(self):
        self.vcan0_bus.send(can.Message(arbitration_id = self.stack_rxid, data =  "\x03\x01\x02\x03", is_extended_id = False))
        frame = self.conn.wait_frame(timeout=1)
        self.assertEqual(frame, "\x01\x02\x03")

    def test_send(self):
        self.conn.send("\xAA\xBB\xCC\xDD\xEE\xFF")
        t1 = time.time()
        msg = self.vcan0_bus.recv(1)
        self.assertIsNotNone(msg)
        self.assertEqual(msg.data, '\x06\xAA\xBB\xCC\xDD\xEE\xFF')

    def test_reopen(self):
        self.conn.send("\x0A\x0B\x0C\x0D")
        self.vcan0_bus.send(can.Message(arbitration_id = self.stack_rxid, data =  "\x03\x01\x02\x03", is_extended_id = False))
        self.conn.close()
...
TestPythonIsoTpConnection = unittest.skipIf(_STACK_POSSIBLE == False, u'Cannot test TestPythonIsoTpConnection. %s' % _STACK_UNVAILABLE_REASON)(TestPythonIsoTpConnection)
