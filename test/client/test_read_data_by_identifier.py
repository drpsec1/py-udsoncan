...
class TestReadDataByIdentifier(ClientServerTest):
    ...
    def test_rdbi_single_success(self):
        ...
        self.assertEqual(request, "\x22\x00\x01")
        self.conn.fromuserqueue.put("\x62\x00\x01\x12\x34")	# Positive response
    ...
    def test_rdbi_multiple_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x22\x00\x01\x00\x02\x00\x04\x00\x03")
        self.conn.fromuserqueue.put("\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x04\x61\x62\x63\x64\x65\x00\x03\x11")	# Positive response
    ...
    def _test_rdbi_multiple_success(self):
        ...
        self.assertEqual(values[4], u'abcde')	


    def test_rdbi_multiple_zero_padding1_success(self):
        data = '\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x03\x11'
        for i in xrange(8):
    ...
    def test_rdbi_multiple_zero_padding_not_tolerated_exception(self):
        ...
        self.wait_request_and_respond(data + "\x00")	# One extra byte is incomplete DID = invalid response

        for i in xrange(1,7):
            self.wait_request_and_respond(data + "\x00"*(i+2))	# 2 extra bytes = valid DID. Make sure to have data in payload as library do not allow empty pack string	
    ...
    def _test_rdbi_multiple_zero_padding_not_tolerated_exception(self):
        ...
        for i in xrange(1,7):
            with self.assertRaises(UnexpectedResponseException):	# Not requested DID 0x0000
                self.udsclient.config[u'data_identifiers'][0] = u'B'*i
                self.udsclient.read_data_by_identifier(didlist = [1,2,3])
    ...
    def _test_rdbi_multiple_zero_padding_not_tolerated_no_exception(self):
        ...
        self.udsclient.config[u'exception_on_unexpected_response'] = True

        response = self.udsclient.read_data_by_identifier(didlist = [1,2,3])
        self.assertFalse(response.valid)

        self.udsclient.config[u'exception_on_invalid_response'] = True
    ...
    def test_rdbi_variable_size_did(self):
        self.wait_request_and_respond("\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x05\xaa\xbb\xcc\xdd")  
    ...
    def _test_rdbi_variable_size_did(self):
        ...
        self.assertEqual(values[1], (0x1234,))      
        self.assertEqual(values[2], (0x7856,))      
        self.assertEqual(values[5], '\xaa\xbb\xcc\xdd') 
    ...
    def test_rdbi_incomplete_response_exception(self):
        self.wait_request_and_respond("\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x03")  
    ...
    def test_rdbi_unknown_did_exception(self):
        self.wait_request_and_respond("\x62\x00\x09\x12\x34\x00\x02\x56\x78\x00\x03\x11")	
    ...
    def test_rdbi_unwanted_did_exception(self):
        self.wait_request_and_respond("\x62\x00\x01\x12\x34\x00\x02\x56\x78\x00\x03\x11")
    ...
    def test_rdbi_invalidservice_exception(self):
        self.wait_request_and_respond("\x00\x00\x01\x12\x34")	# Service is inexistant
    ...
    def test_rdbi_wrongservice_exception(self):
        self.wait_request_and_respond("\x50\x00\x01\x12\x34")	# Valid service, but not the one requested
