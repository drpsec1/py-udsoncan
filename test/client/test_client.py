...
class TestClient(ClientServerTest):
    ...
    def _test_timeout_override(self):
        ...
        try:
            ...
            raise Exception(u'Request did not raise a TimeoutException')
        except TimeoutException, e:
            diff = time.time() - t1
            self.assertGreater(diff, timeout, u'Timeout raised after %.3f seconds when it should be %.3f sec' % (diff, timeout))
            self.assertLess(diff, timeout+0.5, u'Timeout raised after %.3f seconds when it should be %.3f sec' % (diff, timeout))
    ...
    def _test_no_response_overall_timeout(self):
        req = Request(service = services.TesterPresent, subfunction=0) 
        timeout = 0.5
        self.udsclient.set_configs({u'request_timeout': timeout, u'p2_timeout' : 2, u'p2_star_timeout':2})
        try:
            t1 = time.time()
    ...
    def _test_no_response_p2_timeout(self):
        req = Request(service = services.TesterPresent, subfunction=0) 
        timeout = 0.5
        self.udsclient.set_configs({u'request_timeout': 2, u'p2_timeout' : timeout, u'p2_star_timeout':2})
        try:
            t1 = time.time()
            response = self.udsclient.send_request(req)
            raise Exception(u'Request did not raise a TimeoutException')
    ...
    def test_overall_timeout_pending_response(self):
        if not hasattr(self, u'completed'):
    ...
    def _test_overall_timeout_pending_response(self):
        ...
        try:
            ...
            self.assertIsNotNone(self.udsclient.last_response, u'Client never received the PendingResponse message')
            self.completed = True
            diff = time.time() - t1
            self.assertGreater(diff, timeout, u'Timeout raised after %.3f seconds when it should be %.3f sec' % (diff, timeout))
            self.assertLess(diff, timeout+0.5, u'Timeout raised after %.3f seconds when it should be %.3f sec' % (diff, timeout))
    ...
    def _test_p2_star_timeout(self):
        req = Request(service = services.TesterPresent, subfunction=0) 
        timeout = 2
        self.udsclient.set_configs({u'request_timeout': 5, u'p2_timeout' : 0.5, u'p2_star_timeout':timeout})
        try:
            t1 = time.time()
            response = self.udsclient.send_request(req)
            raise Exception(u'Request did not raise a TimeoutException')
    ...
    def test_p2_star_timeout_overrided_by_diagnostic_session_control(self):
        self.conn.touserqueue.get(timeout=0.2)
        self.conn.fromuserqueue.put("\x50\x01\x03\xE8\x00\xC8")  # Respond to diagnostic session control with timeout of 2 sec
    ...
    def _test_p2_star_timeout_overrided_by_diagnostic_session_control(self):
        req = Request(service = services.TesterPresent, subfunction=0) 
        self.udsclient.set_configs({u'request_timeout': 5, u'p2_timeout' : 0.5, u'p2_star_timeout':1})
    ...
    def test_payload_override_literal(self):
        ...
        self.assertEqual(request, '\x12\x34\x56\x78')
        self.conn.fromuserqueue.put("\x7E\x00")

    def _test_payload_override_literal(self):
        req = Request(service = services.TesterPresent, subfunction=0) 
        with self.udsclient.payload_override('\x12\x34\x56\x78'):
            response = self.udsclient.send_request(req)
            self.assertEqual(response.original_payload, '\x7E\x00')


    def test_payload_override_func(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, '\x99\x88\x77\x66')
        self.conn.fromuserqueue.put("\x7E\x00")

    def _test_payload_override_func(self):
        def func(payload):
            return '\x99\x88\x77\x66'
