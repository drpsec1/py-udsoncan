...
class TestAccessTimingParameter(ClientServerTest):
    ...
    def test_read_extended_params_success(self):
        ...
        self.assertEqual(request, "\x83\x01")
        self.conn.fromuserqueue.put("\xC3\x01\x99\x88\x77\x66")	# Positive response
    ...
    def _test_read_extended_params_success(self):
        ...
        self.assertEqual(response.service_data.timing_param_record, "\x99\x88\x77\x66")

    def test_read_extended_params_success_spr(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x83\x81")
        self.conn.fromuserqueue.put(u'wait')
    ...
    def test_read_active_params_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x83\x03")
        self.conn.fromuserqueue.put("\xC3\x03\x99\x88\x77\x66")	# Positive response
    ...
    def test_set_params_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x83\x04\x11\x22\x33\x44")
        self.conn.fromuserqueue.put("\xC3\x04")	# Positive response

    def _test_set_params_success(self):
        self.udsclient.set_timing_parameters(params="\x11\x22\x33\x44")

    def test_set_params_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x83\x02")
        self.conn.fromuserqueue.put("\xC3\x02")	# Positive response

    def _test_set_params_success(self):
        self.udsclient.reset_default_timing_parameters()

    def test_set_params_denied_exception(self):
        self.wait_request_and_respond("\x7F\x83\x31") #Request Out Of Range
    ...
    def test_access_timing_params_bad_access_type_exception(self):
        self.wait_request_and_respond("\xC3\x23") # Valid but access type
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            response = self.udsclient.access_timing_parameter(access_type=services.AccessTimingParameter.AccessType.readExtendedTimingParameterSet, timing_param_record="\xaa\xbb")
