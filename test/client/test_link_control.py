...
class TestLinkContorl(ClientServerTest):
    ...
    def test_linkcontrol_verify_fixed(self):
        ...
        self.assertEqual(request, "\x87\x01\x11")
        self.conn.fromuserqueue.put("\xC7\x01")	# Positive response
    ...
    def test_linkcontrol_verify_fixed_spr(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x87\x81\x11")
    ...
    def test_linkcontrol_verify_specific(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x87\x02\x12\x34\x56")
        self.conn.fromuserqueue.put("\xC7\x02")	# Positive response
    ...
    def test_linkcontrol_verify_specific_from_fixed(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x87\x02\x07\xA1\x20")
        self.conn.fromuserqueue.put("\xC7\x02")	# Positive response
    ...
    def test_linkcontrol_custom_control_type(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x87\x55")
        self.conn.fromuserqueue.put("\xC7\x55")	# Positive response
    ...
    def test_linkcontrol_negative_response_exception(self):
        self.wait_request_and_respond("\x7F\x87\x31") 	# Request Out Of Range
    ...
    def test_linkcontrol_invalidservice_exception(self):
        self.wait_request_and_respond("\x00\x22") 	# Request Out Of Range
    ...
    def test_linkcontrol_bad_control_type_exception(self):
        self.wait_request_and_respond("\xC7\x08") # Valid but bad control type
    ...
    def _test_bad_param(self):
        with self.assertRaises(ValueError):
            self.udsclient.link_control(control_type=u'x')	
        ...
        with self.assertRaises(ValueError):
            self.udsclient.link_control(control_type=1, baudrate=u'x') # Baudrate should be Baudrate instance
