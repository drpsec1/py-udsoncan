...
class TestRequestTransferExit(ClientServerTest):
    ...
    def test_request_transfer_exit_success(self):
        ...
        self.assertEqual(request, "\x37\x12\x34\x56")
        self.conn.fromuserqueue.put("\x77\x89\xab\xcd\xef")	# Positive response

    def _test_request_transfer_exit_success(self):
        response = self.udsclient.request_transfer_exit(data='\x12\x34\x56')
    ...
    def test_request_transfer_exit_no_data_ok(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x37")
        self.conn.fromuserqueue.put("\x77")	# Positive response
    ...
    def test_request_transfer_exit_denied_exception(self):
        self.wait_request_and_respond("\x7F\x37\x24") # reset sequence error
    ...
    def _test_request_transfer_exit_invalidservice_exception(self):
        with self.assertRaises(InvalidResponseException) as handle:
            self.udsclient.request_transfer_exit(data='\x12\x34\x56')
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.request_transfer_exit(data=u"asd")
