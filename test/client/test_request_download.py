...
class TestRequestDownload(ClientServerTest):
    ...
    def test_request_download_success(self):
        ...
        self.assertEqual(request, "\x34\x00\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x74\x20\xab\xcd")	# Positive response
    ...
    def test_request_download_config_format(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x34\x00\x24\x00\x00\x12\x34\x00\xFF")	# dfi = 24 and 0 padding
        self.conn.fromuserqueue.put("\x74\x20\xab\xcd")	# Positive response

    def _test_request_download_config_format(self):
        self.udsclient.set_configs({u'server_address_format':32, u'server_memorysize_format':16})
    ...
    def test_request_download_success_lfid(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x34\x00\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x74\x23\xab\xcd")	# Positive response
    ...
    def test_request_download_success_dfi(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x34\x52\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x74\x20\xab\xcd")	# Positive response
    ...
    def test_incomplete_nblock_response_exception(self):
        self.wait_request_and_respond("\x74\x40\xab\xcd")	# Missing 2 bytes to complete number of block
    ...
    def test_request_download_invalidservice_exception(self):
        self.wait_request_and_respond("\x00\x20\x12\x34") #Inexistent Service
    ...
    def test_request_download_wrongservice_exception(self):
        self.wait_request_and_respond("\x7E\x20\x12\x34") # Valid but wrong service (Tester Present)
    ...
    def _test_bad_params(self):
        ...
        with self.assertRaises(ValueError) as handle:
            self.udsclient.request_download(memory_location=u"asd")
