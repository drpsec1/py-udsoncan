...
class TestRequestUpload(ClientServerTest):
    ...
    def test_request_upload_success(self):
        ...
        self.assertEqual(request, "\x35\x00\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x75\x20\xab\xcd")	# Positive response
    ...
    def test_request_upload_config_format(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x35\x00\x24\x00\x00\x12\x34\x00\xFF")	# dfi = 24 and 0 padding
        self.conn.fromuserqueue.put("\x75\x20\xab\xcd")	# Positive response
    ...
    def test_request_upload_success_lfid(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x35\x00\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x75\x23\xab\xcd")	# Positive response
    ...
    def test_request_upload_success_dfi(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x35\x52\x12\x12\x34\xFF")
        self.conn.fromuserqueue.put("\x75\x20\xab\xcd")	# Positive response
    ...
    def test_incomplete_nblock_response_exception(self):
        self.wait_request_and_respond("\x75\x40\xab\xcd")	# Missing 2 bytes to complete number of block
    ...
    def _test_bad_params(self):
        ...
        with self.assertRaises(ValueError) as handle:
            self.udsclient.request_upload(memory_location=u"asd")
