...
class TestRoutineControl(ClientServerTest):
    ...
    def test_start_routine_success(self):
        ...
        self.assertEqual(request, "\x31\x01\x00\x12\x45\x67\x89\xaa")
        self.conn.fromuserqueue.put("\x71\x01\x00\x12\x99\x88")	# Positive response

    def _test_start_routine_success(self):
        response = self.udsclient.start_routine(routine_id=0x12, data = '\x45\x67\x89\xaa')
        ...
        self.assertEqual(response.service_data.routine_status_record, '\x99\x88')

    def test_start_routine_success_spr(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x31\x81\x00\x12\x45\x67\x89\xaa")
    ...
    def test_stop_routine_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x31\x02\x12\x34\x45\x67\x89\xaa")
        self.conn.fromuserqueue.put("\x71\x02\x12\x34\x99\x88")	# Positive response

    def _test_stop_routine_success(self):
        response = self.udsclient.stop_routine(routine_id=0x1234, data = '\x45\x67\x89\xaa')
        self.assertTrue(response.positive)
    ...
    def test_get_routine_result_success(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x31\x03\x12\x34\x45\x67\x89\xaa")
        self.conn.fromuserqueue.put("\x71\x03\x12\x34\x99\x88")	# Positive response

    def _test_get_routine_result_success(self):
        response = self.udsclient.get_routine_result(routine_id=0x1234, data = '\x45\x67\x89\xaa')
        self.assertTrue(response.positive)
    ...
    def test_routine_control_denied_exception(self):
        self.wait_request_and_respond("\x7F\x31\x72") #General Programming FAilure

    def _test_routine_control_denied_exception(self):
        with self.assertRaises(NegativeResponseException) as handle:
            self.udsclient.routine_control(routine_id=0x1234, control_type=0x11, data='\x99\x88')
    ...
    def _test_routine_control_denied_no_exception(self):
        ...
        response = self.udsclient.routine_control(routine_id=0x1234, control_type=0x11, data='\x99\x88')
    ...
    def test_routine_control_invalidservice_exception(self):
        self.wait_request_and_respond("\x00\x11\x12\x34") #Inexistent Service
    ...
    def test_routine_control_wrongservice_exception(self):
        self.wait_request_and_respond("\x7E\x11\x12\x34") # Valid but wrong service (Tester Present)
    ...
    def test_routine_control_bad_controltype_exception(self):
        self.wait_request_and_respond("\x71\x12\x12\x34") # Valid but wrong service (Tester Present)
    ...
    def test_routine_control_bad_routine_id_exception(self):
        self.wait_request_and_respond("\x71\x11\x12\x35") # Valid but wrong service (Tester Present)
