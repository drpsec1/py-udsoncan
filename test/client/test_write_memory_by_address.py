...
class TestWriteMemoryByAddress(ClientServerTest):
    ...
    def test_4byte_block(self):
        ...
        self.assertEqual(request, "\x3D\x12\x12\x34\x04\x66\x77\x88\x99")
        self.conn.fromuserqueue.put("\x7D\x12\x12\x34\x04")

    def _test_4byte_block(self):
        memloc = MemoryLocation(address=0x1234, memorysize=4, address_format=16, memorysize_format=8)
        response = self.udsclient.write_memory_by_address(memloc, '\x66\x77\x88\x99')
    ...
    def test_config_format(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x3D\x24\x00\x00\x12\x34\x00\x04\x66\x77\x88\x99")
        self.conn.fromuserqueue.put("\x7D\x24\x00\x00\x12\x34\x00\x04")
    ...
    def _test_config_format(self):
        ...
        response = self.udsclient.write_memory_by_address(MemoryLocation(address=0x1234, memorysize=4), '\x66\x77\x88\x99')
        alfid = AddressAndLengthFormatIdentifier(address_format = self.udsclient.config[u'server_address_format'], memorysize_format=self.udsclient.config[u'server_memorysize_format'])
        self.assertEqual(response.service_data.alfid_echo, alfid.get_byte_as_int())
        self.assertEqual(response.service_data.memory_location_echo.address, 0x1234)
        self.assertEqual(response.service_data.memory_location_echo.memorysize, 4)

    def test_4byte_block_client_extra_bytes(self):
        self.wait_request_and_respond('\x7D\x12\x12\x34\x04\x01\x02\x03\x04')

    def _test_4byte_block_client_extra_bytes(self):
        memloc = MemoryLocation(address=0x1234, memorysize=4, address_format=16, memorysize_format=8)
        self.udsclient.write_memory_by_address(memloc, '\x66\x77\x88\x99\xAA\xBB')

    def test_4byte_block_harmless_extra_bytes(self):
        self.wait_request_and_respond('\x7D\x12\x12\x34\x04\x01\x02\x03\x04\x05')
    ...
    def test_bad_echo_ali_exception(self):
        self.wait_request_and_respond('\x7D\x21\x12\x34\x04')
    ...
    def test_bad_echo_address_exception(self):
        self.wait_request_and_respond('\x7D\x12\x12\x35\x04')
    ...
    def test_bad_echo_memorysize_exception(self):
        self.wait_request_and_respond('\x7D\x12\x12\x34\x05')
    ...
    def test_request_denied_exception(self):
        self.wait_request_and_respond("\x7F\x3D\x45") #Request Out Of Range
    ...
    def test_request_invalid_service_exception(self):
        self.wait_request_and_respond("\x00\x12\x12\x34\x04") #Inexistent Service
    ...
    def test_wrong_service_exception(self):
        self.wait_request_and_respond("\x7E\x12\x12\x34\x04") # Valid but wrong service (Tester Present)
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.write_memory_by_address(1, '\x00\x01')

        with self.assertRaises(ValueError):
            self.udsclient.write_memory_by_address(u'aaa',  '\x00\x01')

        with self.assertRaises(ValueError):
            self.udsclient.write_memory_by_address(memloc,  1)

        with self.assertRaises(ValueError):
            self.udsclient.write_memory_by_address(memloc,  u'aaa')
