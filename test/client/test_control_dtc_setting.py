...
class TestControlDTCSettings(ClientServerTest):
    ...
    def test_set_on(self):
        ...
        self.assertEqual(request, "\x85\x01")
        self.conn.fromuserqueue.put("\xC5\x01")	# Positive response
    ...
    def test_set_on_spr(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x85\x81")
    ...
    def test_set_on_with_extra_data(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x85\x01\x11\x22\x33")
        self.conn.fromuserqueue.put("\xC5\x01")	# Positive response

    def _test_set_on_with_extra_data(self):
        response = self.udsclient.control_dtc_setting(setting_type=services.ControlDTCSetting.SettingType.on, data='\x11\x22\x33')
        self.assertEqual(response.service_data.setting_type_echo, services.ControlDTCSetting.SettingType.on)

    def test_set_on_harmless_extra_bytes_in_response(self):
        self.wait_request_and_respond("\xC5\x01\x77\x88\x99")	# Positive response
    ...
    def test_set_params_denied_exception(self):
        self.wait_request_and_respond("\x7F\x85\x45") #Request Out Of Range
    ...
    def test_bad_setting_type_response_exception(self):
        self.wait_request_and_respond("\xC5\x23") # Valid but access type
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.control_dtc_setting(setting_type=services.ControlDTCSetting.SettingType.on, data=u'asdasdasd')
