...
class TestTransferData(ClientServerTest):
    ...
    def test_transfer_data_success(self):
        ...
        self.assertEqual(request, "\x36\x22\x12\x34\x56")
        self.conn.fromuserqueue.put("\x76\x22\x89\xab\xcd\xef")	# Positive response

    def _test_transfer_data_success(self):
        response = self.udsclient.transfer_data(sequence_number=0x22, data='\x12\x34\x56')
        self.assertEqual(response.service_data.sequence_number_echo, 0x22)
        self.assertEqual(response.service_data.parameter_records, '\x89\xab\xcd\xef')
    ...
    def test_transfer_data_no_data_ok(self):
        request = self.conn.touserqueue.get(timeout=0.2)
        self.assertEqual(request, "\x36\x22")
        self.conn.fromuserqueue.put("\x76\x22")	# Positive response

    def _test_transfer_data_no_data_ok(self):
        response = self.udsclient.transfer_data(sequence_number=0x22)
        self.assertEqual(response.service_data.sequence_number_echo, 0x22)
        self.assertEqual(response.service_data.parameter_records, '')	

    def test_transfer_data_denied_exception(self):
        self.wait_request_and_respond("\x7F\x36\x73") # wrong block sequence number
    ...
    def test_transfer_data_bad_sequence_number_exception(self):
        self.wait_request_and_respond("\x76\x23\x89\xab\xcd\xef")	# Positive response

    def _test_transfer_data_bad_sequence_number_exception(self):
        with self.assertRaises(UnexpectedResponseException):
            self.udsclient.transfer_data(sequence_number=0x22, data='\x12\x34\x56')
    ...
    def _test_bad_param(self):
        ...
        with self.assertRaises(ValueError):
            self.udsclient.transfer_data(sequence_number=1, data=u'asd')
