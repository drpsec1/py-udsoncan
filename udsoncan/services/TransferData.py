...
class TransferData(BaseService):
    ...
    def make_request(cls, sequence_number, data=None):
        ...
        ServiceHelper.validate_int(sequence_number, min=0, max=0xFF, name=u'Block sequence counter')	# Not a subfunction!

        if data is not None and not isinstance(data, str):
            raise ValueError(u'data must be a bytes object')

        request = Request(service=cls)
        request.data = struct.pack(u'B', sequence_number)
    ...
    def interpret_response(cls, response):
        ...
        response.service_data.sequence_number_echo = ord(response.data[0])
        response.service_data.parameter_records = response.data[1:] if len(response.data) > 1 else ''
    ...
    class ResponseData(BaseResponseData):
        ...
        """		
        def __init__(self):
            super(TransferData.ResponseData, self).__init__(TransferData)
