...
class ReadDataByIdentifier(BaseService):
    ...
    def validate_didlist_input(cls, dids):
        if not isinstance(dids, (int, long)) and not isinstance(dids, list):
            raise ValueError(u"Data Identifier must either be an integer or a list of integer")

        if isinstance(dids, (int, long)):
            ServiceHelper.validate_int(dids, min=0, max=0xFFFF, name=u'Data Identifier')

        if isinstance(dids, list):
            for did in dids:
                ServiceHelper.validate_int(did, min=0, max=0xFFFF, name=u'Data Identifier')
    ...
    def make_request(cls, didlist, didconfig):
        ...
        for did in didlist:
            if did not in didconfig:    # Already checked in check_did_config. Paranoid check
                raise ConfigError(key=did, msg=u'Actual data identifier configuration contains no definition for data identifier 0x%04x' % did)
                
            codec = DidCodec.from_config(didconfig[did])
            try:
                length = len(codec)
                if did_reading_all_data is not None:
                    raise ValueError(u'Did 0x%04X is configured to read the rest of the payload (__len__ raisong ReadAllRemainingData), but a subsequent DID is requested (0x%04x)' % (did_reading_all_data, did))
            except DidCodec.ReadAllRemainingData:
                if did_reading_all_data is not None:
                    raise ValueError(u'It is impossible to read 2 DIDs configured to read the rest of the payload (__len__ raising ReadAllRemainingData). Dids are : 0x%04X and 0x%04X' % (did_reading_all_data, did))
                did_reading_all_data = did

        req.data = struct.pack(u'>'+u'H'*len(didlist), *didlist) #Encode list of DID
    ...
    def interpret_response(cls, response, didlist, didconfig, tolerate_zero_padding=True):
        ...
        while True:
            ...
            if len(response.data) <= offset +1:
                if tolerate_zero_padding and ord(response.data[-1]) == 0:	# One extra byte, but it's a 0 and we accept that. So we're done
                    break
                raise InvalidResponseException(response, u"Response given by server is incomplete.")

            did = struct.unpack(u'>H', response.data[offset:offset+2])[0]	# Get the DID number
            if did == 0 and did not in didconfig and tolerate_zero_padding: # We read two zeros and that is not a DID bu we accept that. So we're done.
                if response.data[offset:] == '\x00' * (len(response.data) - offset):
                    break
            ...
            if len(response.data) < offset+payload_size:
                raise InvalidResponseException(response, u"Value for data identifier 0x%04x was incomplete according to definition in configuration" % did)
    ...
    class ResponseData(BaseResponseData):
        ...
        """				
        def __init__(self):
            super(ReadDataByIdentifier.ResponseData, self).__init__(ReadDataByIdentifier)
